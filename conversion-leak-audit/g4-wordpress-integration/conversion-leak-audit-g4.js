(function () {
  ...
  var phaseCopy = {
    ...
    MATCHING_EVIDENCE: ['Matching evidence', '正在把观察到的事实与可信规则匹配。'],
    PRIORITIZING: ['Prioritizing', '正在整理最值得先看的问题。']
  };
  var findingMeta = {
    'CORE-007': {
  ...
  function sourceFor(report, refs) {
    ...
    var pageIndex = 0;
    var match = (refs || []).join(' ').match(/page:(\d+)/);
    if (match) pageIndex = Number(match[1]);
    var page = pages[pageIndex] || pages[0];
    return page ? (page.final_url || page.requested_url || 'Audited public page') : 'Structured Scanner evidence';
  }
  ...
  function renderResults(report) {
    ...
    var cards = findings.map(function (finding) {
      ...
      return '<article class="cla-g4-finding" data-rule-id="' + escapeHtml(decision.rule_id) + '">' +
        ...
        '<details data-evidence-detail><summary>View evidence</summary><div class="cla-g4-evidence"><p><strong>Source</strong>' + escapeHtml(source) + '</p><p><strong>Evidence references</strong>' + escapeHtml(refs) + '</p><p><strong>Scanner decision</strong>' + escapeHtml(decision.message) + '</p><p><strong>Limit</strong>Public-page evidence is a useful starting point, not proof of revenue loss or causal lift.</p></div></details>' +
        '</article>';
    }).join('');
    ...
    results.innerHTML = '<div class="cla-g4-results-head"><div><p class="cla-g4-kicker">FREE TOP 3</p><h2>' + escapeHtml(summary) + '</h2><p class="cla-g4-muted">Only evidence-backed ISSUE decisions are shown. The list is never padded.</p></div><span class="cla-g4-result-count">' + count + ' finding' + (count === 1 ? '' : 's') + '</span></div>' +
    ...
    showFeedback('', '');
    emit('scan_completed', publicEventProperties({finding_count: count, top3_available: count > 0}));
    emit('top3_viewed', publicEventProperties({top3_count: count}));
    results.querySelectorAll('[data-evidence-detail]').forEach(function (detail) {
      detail.addEventListener('toggle', function () {
        if (detail.open) {
          var rule = detail.closest('[data-rule-id]');
          emit('issue_expanded', publicEventProperties({rule_id: rule ? rule.getAttribute('data-rule-id') : 'unknown', priority_rank: rule ? rule.querySelector('.cla-g4-priority').textContent : '', finding_type: 'ISSUE'}));
        }
      });
    });
  ...
  function renderIncomplete(job, report) {
    ...
    emit('scan_incomplete', publicEventProperties({reason: job.error_code || 'UNKNOWN_FAILURE'}));
  ...
  async function startScan(url) {
    ...
    try {
      ...
      emit('scan_started', publicEventProperties({normalized_host_class: 'public_host', scan_id: scanId}));
      setProgress(job);
      await poll(scanId);
    } catch (error) {
  ...
  emit('landing_view', publicEventProperties({locale: document.documentElement.lang || 'unknown'}));
}());
