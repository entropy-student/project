# Daily Topic / Publishing Planner v0.2 — PROPOSED

> **NOT ACTIVE until Owner PASS of G3R.**  
> The currently scheduled Daily Topic Radar keeps using the accepted v0.1 behavior until then.

## Proposed changes after PASS

1. Keep Daily Radar at the current daily morning cadence.
2. Treat Owner decision as:
   `EDITORIAL_CADENCE_TARGET = DAILY`.
3. Maintain one planned Bilibili episode per calendar day.
4. Add `editorial_mode` to every planned slot:
   - STORY_MODEL
   - STORY_ACTION
   - STORY_TUTORIAL
5. Initial seven-day portfolio target:
   - 4 STORY_MODEL
   - 2 STORY_ACTION
   - 1 STORY_TUTORIAL / real experiment.
6. A qualified HOT topic may override any unlocked slot regardless of mode; the displaced Evergreen returns to the bank.
7. Do not force the weekly ratio when good candidates are missing. Quality gates outrank quota.
8. Suggested duration is routed by mode:
   - HOT: 3–5 min;
   - standard STORY_MODEL: 4–6 min;
   - STORY_ACTION / STORY_TUTORIAL: 5–8 min.
9. Calendar should show:
   Date / Topic / Lane / Business Job / Editorial Mode / Target Duration / Status / Notes.
10. Daily Radar still does **not** publish, enter G4+, or claim production throughput.

## Important distinction

Daily selection and daily publishing are editorial goals.

The production system must later demonstrate repeatable throughput before:
`DAILY_PRODUCTION_CAPABILITY = PASS`.
