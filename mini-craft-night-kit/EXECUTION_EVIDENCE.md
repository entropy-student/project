...
## K6_PHASE_F_R1R1_NATIVE_READONLY_CADDY_SOURCE_RECONCILIATION — RETURN (2026-09-27)

```text
GATE=K6_PHASE_F_R1R1_NATIVE_READONLY_CADDY_SOURCE_RECONCILIATION
RESULT=RETURN_REVIEWER_F_R1R1_READONLY_PROBE_FAILED
SUMMARY=Exactly one canonical strict SSH session was established and the pre-sudo remote identity matched ops@srv1970241. Fresh container state reads passed (Caddy/WordPress/MariaDB running; restart counts 0; MariaDB healthy). The first published-port metadata probe failed in docker inspect --format with native subcommand exit 64 at WP_PORTS. Per Gate boundary, no retry or further remote substep was run.
SSH_ATTEMPTS=1
SSH_NATIVE_EXIT=70;REMOTE_PAYLOAD_RETURNED_70
SSH_HOST_KEY_MATCH=YES;STRICT_KNOWN_HOSTS_HANDSHAKE_ESTABLISHED
REMOTE_IDENTITY=ops@srv1970241
REMOTE_PRE_SUDO_WHOAMI=ops
REMOTE_PRE_SUDO_ID_UN=ops
REMOTE_PRE_SUDO_UID_NONZERO=YES;UID=1000
REMOTE_PRE_SUDO_HOSTNAME=srv1970241
CADDY_WORDPRESS_MARIADB_CONTINUITY=PASS;ALL_RUNNING;RESTART_COUNTS=0;MARIADB=HEALTHY
WP_PORTS_PROBE=RETURN;DOCKER_INSPECT_FORMAT_NATIVE_EXIT=64
DB_PUBLIC_PORT=NOT_VERIFIED_AFTER_EARLY_RETURN
CADDY_80_443_OWNER=NOT_VERIFIED_AFTER_EARLY_RETURN
EDGE_TO_MINICRAFT_PRIVATE_REACHABILITY=NOT_RUN_AFTER_EARLY_RETURN
CURRENT_MINICRAFT_DNS=NXDOMAIN;FRESH_GOOGLE_DOH_A_AAAA_CNAME_STATUS=3
...
CADDY_SOURCE_RECONCILIATION=NOT_COMPLETED
CANARY_INDEXING_STATE=NOT_CHECKED
